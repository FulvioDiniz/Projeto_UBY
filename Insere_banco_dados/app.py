...
@app.route('/')
def index():
    """Página inicial: Lista todas as receitas cadastradas."""
    conn = get_db_conn()
    receitas = []
    if conn:
        cursor = conn.cursor()
...
def criar_receita():
    """Página para criar uma nova receita."""
    if request.method == 'POST':
        ...
        if conn:
            ...
            try:
                ...
                flash(f'Erro: A Receita com ID {receita_id} já existe.', 'danger')
            except Exception as e:
...
def detalhe_receita(receita_id):
    """Página que mostra os detalhes de uma receita específica."""
    conn = get_db_conn()
    receita = None
    produtos_na_receita = []
    produtos_globais = []
    ...
    return render_template('detalhe_receita.html', receita=receita, produtos_na_receita=produtos_na_receita, produtos_globais=produtos_globais)

@app.route('/receita/adicionar_produto', methods=['POST'])
def adicionar_produto_receita():
    """Lógica para associar um ProdutoGlobal a uma Receita."""
    receita_id = request.form['receita_id']
    ...
    if conn:
        ...
        try:
            ...
            flash('Este produto já está associado a esta receita.', 'warning')
        except Exception as e:
...
def adicionar_lote():
    """Lógica FINAL para adicionar um Lote usando a cláusula OUTPUT."""
    # Dados do formulário
    receita_id = request.form['receita_id']
    produto_id = request.form['produto_id']
    ...
    if conn:
        ...
        try:
            ...
            # 1. Inserir o Lote e já capturar o ID de retorno com OUTPUT
            ...
            # 2. Verificar se o lote_id foi retornado corretamente antes de prosseguir
            if not lote_id:
                raise Exception("Falha ao obter o ID do novo lote. A inserção foi cancelada.")

            # 3. Associar o lote à receita na tabela ReceitaProdutoLote
            ...
            # 4. Se tudo deu certo, salvar as alterações
            conn.commit()
...
# --- Rotas para Gerenciar Produtos (sem alterações) ---
@app.route('/produtos')
def gerenciar_produtos():
    """Página para listar e adicionar Produtos Globais."""
    conn = get_db_conn()
    produtos = []
    if conn:
        cursor = conn.cursor()
...
def adicionar_produto_global():
    """Lógica para inserir um novo produto na tabela ProdutoGlobal."""
    ...
    if conn:
        ...
        try:
            ...
            flash(f'Erro: O número de produto "{numero_produto}" já existe.', 'danger')
        except Exception as e:
